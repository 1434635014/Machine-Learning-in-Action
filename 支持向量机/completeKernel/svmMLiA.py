...
def smoSimple(dataMatIn,classLabels,C,toler,maxIter):#输入分别为数据集，类标签，常数C,容许错误率，退出前最大的循环次数
    ...
    while (iter<maxIter):#迭代次数大于最大迭代次数时，退出迭代
        ...
        for i in range(m):#循环每个数据
            # 步骤1：计算误差Ei
            fXi=float(numpy.multiply(alphas,labelMat).T*(dataMatrix*dataMatrix[i,:].T))+b#预测第i个数据的类别  numpy.multiply对应的位置相乘
...
def kernelTrans(X,A,kTup):
    ...
    else: raise NameError('Houston we have a problem--that kernel is not recognized')#核函数不是我们这儿已有的
    return K
def testRbf(k1=1.3):
...
def testDigits(kTup=('rbf', 10)):
    dataArr,labelArr = loadImages('../data/trainingDigits') #加载训练数据
    ...
    dataArr,labelArr = loadImages('../data/testDigits')#加载测试数据
    errorCount = 0
    datMat=numpy.mat(dataArr); labelMat = numpy.mat(labelArr).transpose()
    m,n = numpy.shape(datMat)
    for i in range(m):
...
if __name__ == '__main__':
    ...
    b, alphas = smoP(dataArr, labelArr, 200, 0.0001, 10000, ('rbf', k1)) #C=200 important
    # w = calculateW(alphas, dataArr, labelArr)
    # drawing(np.array(dataArr), np.array(labelArr), alphas, w, b)
